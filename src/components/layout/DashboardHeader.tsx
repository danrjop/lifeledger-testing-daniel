...
export default function DashboardHeader({ onUploadClick, children }: DashboardHeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center gap-4 border-b border-bg-tertiary/50 bg-bg-primary/80 backdrop-blur-md px-8 py-3">
      {/* Canned-query dropdown — demo replacement for live search */}
      <div className="flex-1 max-w-2xl mx-auto">
        <CannedQueryDropdown
          placeholder="Pick an example question…"
          onSelect={(q) => router.push(`/dashboard/search?q=${encodeURIComponent(q)}`)}
        />
      </div>
